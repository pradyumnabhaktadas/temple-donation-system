...
One row per Zoho form that takes money, saying which campaign its
donations belong to. That, plus the transaction ID Zoho sends, is what
lets reconciliation issue receipts without a person.

A page rather than environment settings: there are six forms already and a
new one appears whenever the temple runs a programme, so adding a seminar
shouldn't need a redeploy -- and whoever later wonders why a form's
donations aren't being receipted should be able to see the answer.
"""
import os
import sys
...
class TestEditing:
    def test_a_campaign_can_be_set_later(self, client, app):
        """A form added before anyone decided which campaign it belongs to
        reports its payments until that's filled in."""
        from models import ZohoForm
